...
fn xor_cipher(data: &[u8], key: u8) -> Vec<u8> {
    // Applies a simple XOR cipher for test-only encryption/decryption.
...
async fn spawn_bridge_server(app: Router) -> String {
    // Spawns the bridge HTTP server and returns its base URL.
...
#[tokio::test]
async fn full_identity_bridge_mls_flow() {
    // Exercises identity exchange, request/grant puzzle flow, MLS setup, DHT+bridge delivery, and revocation checks.
    let tmp = tempdir().expect("tempdir");
    let clock = Arc::new(FixedClock { now: 1_700_000_000 });
    let state = init_state_with_clock(tmp.path().join("bridge.db"), clock).expect("state");
    let app = app(state);
    ...
    let cipher_key = 0x5a;
    let ciphertext = xor_cipher(plaintext, cipher_key);
    let ciphertext_digest = hash_bytes(HashId::Sha256, &ciphertext);
    let signature = ed25519_sign_hash(&alice.signing_key, &ciphertext_digest)
        .to_bytes()
        .to_vec();
    ...
    let decrypted = xor_cipher(&ciphertext, cipher_key);
    ...
    let revoked_key_hash = hash_bytes(HashId::Sha256, &alice.verifying_key);
    let revocation = RevocationDeclaration {
        user_id: alice.user_id.clone(),
        revoked_key_hash: revoked_key_hash.clone(),
        revoked_at: 1_700_000_100,
        recovery_key_hash: None,
        reason: Some("compromised".to_string()),
    };
    let root = log
        .append_with_root(CheckpointEntry::Revocation(revocation.clone()))

...
fn test_connection_open_performance() {
    ...
    println!(
        "Connection::open baseline: {:?} total ({:?} per open)",
        duration,
        duration / iterations
    );
    ...
    for _ in 0..iterations {
        let conn = pool
            .pop()
            .unwrap_or_else(|| Connection::open(&db_path).unwrap());
        // simulate use
        let _ = &conn;
        pool.push(conn);
    }
    ...
    println!(
        "Simulated pool: {:?} total ({:?} per acquire/release)",
        duration_pool,
        duration_pool / iterations
    );

    if duration_pool > Duration::ZERO {
        println!(
            "Estimated improvement: {:.2}x",
            duration.as_secs_f64() / duration_pool.as_secs_f64()
        );
    }
}

...
pub fn create_identity() -> IdentityState {
    let mut seed = [0u8; 32];
    SysRng.try_fill_bytes(&mut seed).expect("SysRng failed");
...
pub fn random_bytes(len: usize) -> Vec<u8> {
    ...
    SysRng.try_fill_bytes(&mut buffer).expect("SysRng failed");
    buffer
}
...
fn encrypt_state_file(state: &LocalState) -> Result<String, ClientError> {
    ...
    SysRng.try_fill_bytes(&mut nonce).expect("SysRng failed");

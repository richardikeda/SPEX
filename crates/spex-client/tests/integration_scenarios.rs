use spex_client::{
    ...
    redeem_contact_card_to_state, LocalState,
    receive_inbox_messages, ThreadState
};
...
fn deliver_envelope_to_client(recipient_state: &mut LocalState, envelope: &spex_core::types::Envelope) {
    ...
    let recipient_id_bytes = hex::decode(&recipient_state.identity.as_ref().unwrap().user_id_hex).unwrap();
...
async fn test_three_party_conversation() {
    ...
    let bob_thread = serde_json::from_value::<ThreadState>(serde_json::to_value(alice_thread).unwrap()).unwrap();
    let carol_thread = serde_json::from_value::<ThreadState>(serde_json::to_value(alice_thread).unwrap()).unwrap();
    ...
    let (envelope, _manifest, _chunks) = spex_client::send_thread_message(&alice, alice_thread_state, b"Hello Group!").unwrap();
    ...
    let bob_results = receive_inbox_messages(&mut bob_state, &bob_seed, None).await.unwrap();
    assert_eq!(bob_results.items.len(), 1);
    assert_eq!(String::from_utf8(bob_results.items[0].plaintext.clone()).unwrap(), "Hello Group!");

    // 6. Carol receives
    let carol_seed = hex::decode(&carol.user_id_hex).unwrap();
    let carol_results = receive_inbox_messages(&mut carol_state, &carol_seed, None).await.unwrap();
    assert_eq!(carol_results.items.len(), 1);
    assert_eq!(String::from_utf8(carol_results.items[0].plaintext.clone()).unwrap(), "Hello Group!");
}
